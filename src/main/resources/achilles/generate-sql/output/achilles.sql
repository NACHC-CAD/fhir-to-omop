...
into #s_tmpach_11
from synthea_micro.dbo.person P
where not exists
(
  select 1
  from synthea_micro.dbo.death D
...
WITH rawData AS (
  ...
  from synthea_micro.dbo.person p1
    inner join (select person_id, MIN(observation_period_start_date) as index_date from synthea_micro.dbo.observation_period group by PERSON_ID) op1
...
select p.person_id, 
  ...
  from synthea_micro.dbo.person p
  JOIN synthea_micro.dbo.observation_period op on p.person_id = op.person_id
...
(
  ...
  from synthea_micro.dbo.observation_period op
) A
where rn = 1;
...
JOIN synthea_micro.dbo.person p on op.person_id = p.person_id
where op.rn = 1
;
  ...
  from synthea_micro.dbo.person p1
    ...
    (select person_id,
      ...
      ROW_NUMBER() over (PARTITION by person_id order by observation_period_start_date asc) as rn1
       from synthea_micro.dbo.observation_period
    ) op1
    on p1.PERSON_ID = op1.PERSON_ID
    where op1.rn1 = 1
...
select obs_year,
  ...
  from ymd
 where obs_year >= (select min(year(observation_period_start_date)) from synthea_micro.dbo.observation_period)
   and obs_year <= (select max(year(observation_period_start_date)) from synthea_micro.dbo.observation_period)
) 
SELECT 
	109                               AS analysis_id,  
...
INTO 
	#s_tmpach_109
FROM 
	synthea_micro.dbo.observation_period op
CROSS JOIN 
	year_ranges yr
WHERE
...
into #s_tmpach_110
FROM
synthea_micro.dbo.observation_period op1
join 
(
  SELECT DISTINCT 
...
into #s_tmpach_113
from
	(select person_id, COUNT_BIG(OBSERVATION_period_start_date) as num_periods from synthea_micro.dbo.observation_period group by PERSON_ID) op1
...
INTO
  #temp_dates_116
from 
  synthea_micro.dbo.observation_period
;
...
left join
  (select t2.obs_month, op2.*
    from synthea_micro.dbo.observation_period op2, date_keys t2
...
INTO 
	#s_tmpach_200
FROM 
	synthea_micro.dbo.visit_occurrence vo
JOIN 
	synthea_micro.dbo.observation_period op 
ON 
...
into #s_tmpach_207
from
	synthea_micro.dbo.visit_occurrence vo1
	left join synthea_micro.dbo.person p1
...
from
	synthea_micro.dbo.visit_occurrence vo1
	left join synthea_micro.dbo.care_site cs1
...
(
  ...
  from synthea_micro.dbo.visit_occurrence vo inner join 
  synthea_micro.dbo.observation_period op on vo.person_id = op.person_id
...
into #s_tmpach_300
from synthea_micro.dbo.provider;
...
select 303 as analysis_id,
       ...
       count_big(*) AS count_value
  into #s_tmpach_303 
  from synthea_micro.dbo.provider p
  join synthea_micro.dbo.visit_occurrence vo
...
INTO 
	#s_tmpach_400
FROM 
	synthea_micro.dbo.condition_occurrence co
JOIN 
	synthea_micro.dbo.observation_period op 
ON 
...
JOIN (
	...
	FROM 
		synthea_micro.dbo.drug_exposure de
	JOIN 
		synthea_micro.dbo.observation_period op 
	ON 
...
JOIN (
	...
	FROM 
		synthea_micro.dbo.procedure_occurrence po
	JOIN 
		synthea_micro.dbo.observation_period op 
	ON 
...
JOIN (
	...
	FROM 
		synthea_micro.dbo.observation o
	JOIN 
		synthea_micro.dbo.observation_period op 
	ON 
...
into #s_tmpach_814
from
	synthea_micro.dbo.observation o1
...
INTO 
	#s_tmpach_1000
FROM 
	synthea_micro.dbo.condition_era ce
JOIN 
	synthea_micro.dbo.observation_period op 
ON 
...
WITH rawData AS (
  select
    ...
    COUNT_BIG(distinct person_id) as count_value
  from synthea_micro.dbo.person p1
    inner join synthea_micro.dbo.location l1
...
WITH rawData AS (
  ...
  from synthea_micro.dbo.care_site cs1
    inner join synthea_micro.dbo.location l1
...
into #s_tmpach_1200
from synthea_micro.dbo.person p1
	inner join synthea_micro.dbo.care_site cs1
...
into #s_tmpach_1201
from synthea_micro.dbo.visit_occurrence vo1
	inner join synthea_micro.dbo.care_site cs1
	on vo1.care_site_id = cs1.care_site_id
where vo1.care_site_id is not null
	and cs1.place_of_service_concept_id is not null
...
INTO 
	#s_tmpach_1300
FROM
	synthea_micro.dbo.visit_detail vd
JOIN 
    synthea_micro.dbo.observation_period op 
ON 
...
FROM (
	...
	SELECT 
		...
		COUNT(*) record_count
	FROM 
		synthea_micro.dbo.device_exposure de
	LEFT JOIN 
		synthea_micro.dbo.visit_detail vd 
	ON 
		de.visit_occurrence_id = vd.visit_occurrence_id
	...
	SELECT 
		...
		COUNT(*) record_count
	FROM 
		synthea_micro.dbo.measurement m
	LEFT JOIN 
		synthea_micro.dbo.visit_detail vd 
	ON 
  ...
  from synthea_micro.dbo.person p1
	...
	(select person_id, 
		...
		ROW_NUMBER() over (PARTITION by person_id order by payer_plan_period_start_date asc) as rn1
		 from synthea_micro.dbo.payer_plan_period
	) ppp1
...
INTO
  #temp_dates_1409
from 
  synthea_micro.dbo.payer_plan_period
;
...
from
	synthea_micro.dbo.person p1
	inner join 
    synthea_micro.dbo.payer_plan_period ppp1
	on p1.person_id = ppp1.person_id
	,
	#temp_dates_1409 t1 
...
from
	synthea_micro.dbo.person p1
	inner join synthea_micro.dbo.payer_plan_period ppp1
	on p1.person_id = ppp1.person_id
...
from
	synthea_micro.dbo.person p1
	inner join (select person_id, COUNT_BIG(payer_plan_period_start_date) as num_periods from synthea_micro.dbo.payer_plan_period group by PERSON_ID) ppp1
	on p1.person_id = ppp1.person_id
...
cnt as count_value
 into #s_tmpach_1900
 from (
  select 'measurement' as table_name, 'measurement_source_value' as column_name, measurement_source_value as source_value, count_big(*) as cnt from synthea_micro.dbo.measurement where measurement_concept_id = 0 group by measurement_source_value 
  union
  select 'measurement' as table_name, 'unit_source_value' as column_name, unit_source_value as source_value, count_big(*) as cnt from synthea_micro.dbo.measurement where unit_concept_id = 0 group by unit_source_value 
  union
  select 'procedure_occurrence' as table_name,'procedure_source_value' as column_name, procedure_source_value as source_value, count_big(*) as cnt from synthea_micro.dbo.procedure_occurrence where procedure_concept_id = 0 group by procedure_source_value 
  union
  select 'procedure_occurrence' as table_name,'modifier_source_value' as column_name, modifier_source_value as source_value, count_big(*) as cnt from synthea_micro.dbo.procedure_occurrence where modifier_concept_id = 0 group by modifier_source_value 
  union
  select 'drug_exposure' as table_name, 'drug_source_value' as column_name, drug_source_value as source_value, count_big(*) as cnt from synthea_micro.dbo.drug_exposure where drug_concept_id = 0 group by drug_source_value 
  union
  select 'drug_exposure' as table_name, 'route_source_value' as column_name, route_source_value as source_value, count_big(*) as cnt from synthea_micro.dbo.drug_exposure where route_concept_id = 0 group by route_source_value 
  union
  select 'condition_occurrence' as table_name, 'condition_source_value' as column_name, condition_source_value as source_value, count_big(*) as cnt from synthea_micro.dbo.condition_occurrence where condition_concept_id = 0 group by condition_source_value 
  union
  select 'condition_occurrence' as table_name, 'condition_status_source_value' as column_name, condition_status_source_value as source_value, count_big(*) as cnt from synthea_micro.dbo.condition_occurrence where condition_status_concept_id = 0 group by condition_status_source_value 
  union
  select 'observation' as table_name, 'observation_source_value' as column_name, observation_source_value as source_value, count_big(*) as cnt from synthea_micro.dbo.observation where observation_concept_id = 0 group by observation_source_value                  
  union
  select 'observation' as table_name, 'unit_source_value' as column_name, unit_source_value as source_value, count_big(*) as cnt from synthea_micro.dbo.observation where unit_concept_id = 0 group by unit_source_value                  
  union
  select 'observation' as table_name, 'qualifier_source_value' as column_name, qualifier_source_value as source_value, count_big(*) as cnt from synthea_micro.dbo.observation where qualifier_concept_id = 0 group by qualifier_source_value
  union
  select 'payer_plan_period' as table_name, 'payer_source_value' as column_name, payer_source_value as source_value, count_big(*) as cnt from synthea_micro.dbo.payer_plan_period where payer_concept_id = 0 group by payer_source_value                    
  union
  select 'payer_plan_period' as table_name, 'plan_source_value' as column_name, plan_source_value as source_value, count_big(*) as cnt from synthea_micro.dbo.payer_plan_period where plan_concept_id = 0 group by plan_source_value                    
  union
  select 'payer_plan_period' as table_name, 'sponsor_source_value' as column_name, sponsor_source_value as source_value, count_big(*) as cnt from synthea_micro.dbo.payer_plan_period where sponsor_concept_id = 0 group by sponsor_source_value                    
  union
  select 'payer_plan_period' as table_name, 'stop_reason_source_value' as column_name, stop_reason_source_value as source_value, count_big(*) as cnt from synthea_micro.dbo.payer_plan_period where stop_reason_concept_id = 0 group by stop_reason_source_value                    
  union
  select 'provider' as table_name, 'specialty_source_value' as column_name, specialty_source_value as source_value, count_big(*) as cnt from synthea_micro.dbo.provider where specialty_concept_id = 0 group by specialty_source_value
  union  
  select 'provider' as table_name, 'gender_source_value' as column_name, gender_source_value as source_value, count_big(*) as cnt from synthea_micro.dbo.provider where gender_concept_id = 0 group by gender_source_value
  union  
  select 'person' as table_name, 'gender_source_value' as column_name, gender_source_value as source_value, count_big(*) as cnt from synthea_micro.dbo.person where gender_concept_id = 0 group by gender_source_value                    
  union
  select 'person' as table_name, 'race_source_value' as column_name, race_source_value as source_value, count_big(*) as cnt from synthea_micro.dbo.person where race_concept_id = 0 group by race_source_value                    
  union
  select 'person' as table_name, 'ethnicity_source_value' as column_name, ethnicity_source_value as source_value, count_big(*) as cnt from synthea_micro.dbo.person where ethnicity_concept_id = 0 group by ethnicity_source_value                    
  union
  select 'specimen' as table_name, 'specimen_source_value' as column_name, specimen_source_value as source_value, count_big(*) as cnt from synthea_micro.dbo.specimen where specimen_concept_id = 0 group by specimen_source_value                    
  union
  select 'specimen' as table_name, 'unit_source_value' as column_name, unit_source_value as source_value, count_big(*) as cnt from synthea_micro.dbo.specimen where unit_concept_id = 0 group by unit_source_value                    
  union
  select 'specimen' as table_name, 'anatomic_site_source_value' as column_name, anatomic_site_source_value as source_value, count_big(*) as cnt from synthea_micro.dbo.specimen where anatomic_site_concept_id = 0 group by anatomic_site_source_value                    
  union
  select 'specimen' as table_name, 'disease_status_source_value' as column_name, disease_status_source_value as source_value, count_big(*) as cnt from synthea_micro.dbo.specimen where disease_status_concept_id = 0 group by disease_status_source_value                    
  
  union
  select 'visit_occurrence' as table_name, 'visit_source_value' as column_name, visit_source_value as source_value, count_big(*) as cnt from synthea_micro.dbo.visit_occurrence where visit_concept_id = 0 group by visit_source_value
  union
  
	select 'visit_occurrence' as table_name, 'admitted_from_source_value' as column_name, admitted_from_source_value as source_value, count_big(*) as cnt from synthea_micro.dbo.visit_occurrence where  admitted_from_concept_id = 0 group by admitted_from_source_value
  union
  select 'visit_occurrence' as table_name, 'discharged_to_source_value' as column_name, discharged_to_source_value as source_value, count_big(*) as cnt from synthea_micro.dbo.visit_occurrence where discharged_to_concept_id = 0 group by discharged_to_source_value
			
  union
  select 'device_exposure' as table_name, 'device_source_value' as column_name, device_source_value as source_value, count_big(*) as cnt from synthea_micro.dbo.device_exposure where device_concept_id = 0 group by device_source_value
  union
  select 'death' as table_name, 'cause_source_value' as column_name, cause_source_value as source_value, count_big(*) as cnt from synthea_micro.dbo.death where cause_concept_id = 0 group by cause_source_value
) a
where cnt >= 1 
;
...
select distinct person_id into #conoc from synthea_micro.dbo.condition_occurrence;
select distinct person_id into #drexp from synthea_micro.dbo.drug_exposure;
select distinct person_id into #dvexp from synthea_micro.dbo.device_exposure;
select distinct person_id into #msmt from synthea_micro.dbo.measurement;
select distinct person_id into #death from synthea_micro.dbo.death;
select distinct person_id into #prococ from synthea_micro.dbo.procedure_occurrence;
select distinct person_id into #obs from synthea_micro.dbo.observation;
...
select 2004 as analysis_id,
  ...
  (select count(distinct(person_id)) as totalPersons from synthea_micro.dbo.person) totalPersonsDb UNION ALL
select 2004 as analysis_id,
...
select 2004 as analysis_id,
  ...
  (select count(distinct(person_id)) as totalPersons from synthea_micro.dbo.person) as totalPersonsDb) select * INTO #s_tmpach_2004 from rawData;
...
JOIN (
	...
	FROM 
		synthea_micro.dbo.device_exposure d
	JOIN 
		synthea_micro.dbo.observation_period op 
	ON 
		d.person_id = op.person_id
...
;
  ...
  IF OBJECT_ID('synthea_micro_ach_res.dbo.achilles_results', 'U') IS NOT NULL
    drop table synthea_micro_ach_res.dbo.achilles_results;

--HINT DISTRIBUTE_ON_KEY(analysis_id)

select analysis_id, stratum_1, stratum_2, stratum_3, stratum_4, stratum_5, count_value

  into synthea_micro_ach_res.dbo.achilles_results
...
;
  ...
  IF OBJECT_ID('synthea_micro_ach_res.dbo.achilles_results_dist', 'U') IS NOT NULL
    drop table synthea_micro_ach_res.dbo.achilles_results_dist;
...
select analysis_id, stratum_1, stratum_2, stratum_3, stratum_4, stratum_5, count_value, min_value, max_value, avg_value, stdev_value, median_value, p10_value, p25_value, p75_value, p90_value
  ...
  into synthea_micro_ach_res.dbo.achilles_results_dist
...
create index idx_ar_s1 on synthea_micro_ach_res.dbo.achilles_results (stratum_1);

create index idx_ar_s2 on synthea_micro_ach_res.dbo.achilles_results (stratum_2);

create index idx_ar_aid_s1 on synthea_micro_ach_res.dbo.achilles_results (analysis_id,stratum_1);

create index idx_ar_aid_s1234 on synthea_micro_ach_res.dbo.achilles_results (analysis_id,stratum_1,stratum_2,stratum_3,stratum_4);

create index idx_ard_aid on synthea_micro_ach_res.dbo.achilles_results_dist (analysis_id);

create index idx_ard_s1 on synthea_micro_ach_res.dbo.achilles_results_dist (stratum_1);

create index idx_ard_s2 on synthea_micro_ach_res.dbo.achilles_results_dist (stratum_2);